/**************************************************************************************************
	** File Name: project7BMain.cpp
	** Author: Devin Swanson
	** Due Date: 06/08/2021
	** Description: This is the main file for the autocorrelation project to find the hidden 
			sine wave in an original signal.

	** NOTE: The code below is the skeleton code provided by Professor Bailey
**************************************************************************************************/
...
//only do this many shifts, not all NUMELEMENTS of them (this is enough to uncover secret sine wave)
...
//pick which file type to read, BINARY or ASCII (Binary much faster to read)
...
int main(int argc, char* argv[]) {
	...
	PPSums = new float[MAXSHIFTS];				//per-processor local sums of products

	//read the BigSignal array

	if (me == BOSS) {							//this is the big-data-owner
		BigSignal = new float[NUMELEMENTS + MAXSHIFTS];		//so we can duplicate part of array
	...
	if (me == BOSS) {
		for (int dst = 0; dst < NumCpus; dst++) {
			...
			//MPI send instruction goes here: 
			MPI_Send(&BigSignal[dst * PPSize], PPSize, MPI_FLOAT, dst, 'D', MPI_COMM_WORLD);
		}
	}
	else {
		//perform MPI_Recv here:
		MPI_Recv(PPSignal, PPSize, MPI_FLOAT, BOSS, 'D', MPI_COMM_WORLD, &status);
	}
	...
	if (me == BOSS) {
		for (int s = 0; s < MAXSHIFTS; s++) {
			BigSums[s] = PPSums[s];				//start the overall sums with BOSS's sums
		}
	}
	else {
		//Another MPI_Send command:
		MPI_Send(PPSums, MAXSHIFTS, MPI_FLOAT, BOSS, 'A', MPI_COMM_WORLD);
	}
	...
	if (me == BOSS) {
		...
		for (int src = 0; src < NumCpus; src++) {
			...
			//Another MPI_Recv call goes here:
			MPI_Recv(tmpSums, MAXSHIFTS, MPI_FLOAT, src, 'A', MPI_COMM_WORLD, &status);
			for (int s = 0; s < MAXSHIFTS; s++) {
				BigSums[s] += tmpSums[s];
			}
		}
	...
	if (me == BOSS) {
		...
		fprintf(stderr, "%3d processors, %10d elements, %9.2lf mega-autocorrelations computer per second\n",
...
//read from per-processor signal array, write into the local sums array
